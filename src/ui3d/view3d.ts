 ...
 *
 * The tab runs the 4Hi only (since 2026-09-19). `sim3d` still builds and
 * solves the 2Hi, 6Hi, 12Hi and 20Hi - the node checks under tools/sim3d
...
export function installView3D(root: HTMLElement, opts: { onMesh?: (text: string, detail: string) => void } = {}): View3DHandle {
  const params: Params3D = defaultParams('4hi');
  ...
  /* ── the FrontISTR cross-check: once a solve has converged, the same strip load on the work
     and backup rolls as solids in contact, solved by fistr1 through the dev server's bridge
     (frontistr.ts). Their deflections, the contact load and the exit profile are drawn over
     the model's and set beside them. ── */
  ...
  const fistrSec = section('FrontISTR 照合', {
    ...
    hint: 'このモデル（ティモシェンコ梁 ＋ Hertz の接触・扁平）と、上半分の WR と BUR をソリッド要素で解いた FrontISTR の解（ロール同士は接触解析）。板の荷重 q(x)（収束したスライスの値）を両方に同じだけ与えるので、違いはロールの力学だけ。'
      + '撓みは BUR の軸受に対する各ロール軸の値（板中央）、線荷重は WR–BUR の接触（WR の節点の接触法線力を駅ごとに集める）、'
      + '出側プロファイルは WR 下面の変位から（板の荷重が駅ごとの節点リングに載るので板端付近は粗い）。「最大差」の括弧はモデルの最大値に対する比。'
      + '格子は関門と同じ 81 点・弧 8 分割で、ロールは粗い網（接触の非線形で数分）。詳細は tools/frontistr/README.md。',
  });
  ...
  const sideSec = section('側面図（横から）', {
    open: true, onToggle: redrawOnOpen,
    hint: '端面図のロール配置を横（オペレータ側）から見た図。上半分の WR と BUR の胴・ネックを実寸の比で、支持（□ チョック、赤 ▼ 圧下、緑の矢印 ベンダー）とパスラインの板幅とともに。ハウジング変形考慮モードではハウジング枠も。撓みは正面図、荷重は端面図。',
  });
  ...
  /**
   * Dimensions the other settings bound: a roll's neck no thicker than the roll, its support
   * span no shorter than its barrel - a bearing inside the barrel is no mill. The
  ...
  const BOUNDS: Bound[] = [
    { key: 'wrDn', by: 'wrD', side: 'max' }, { key: 'burDn', by: 'burD', side: 'max' },
    { key: 'wrLs', by: 'wrLb', side: 'min' }, { key: 'burLs', by: 'burLb', side: 'min' },
  ];
  ...
  const buildLeft = () => {
    ...
    numSec.body.append(toggle('扁平の幅方向の広がり（非局所）', params.flatNonlocal, (v) => { params.flatNonlocal = v; apply(); },
      ...
      + 'ON が既定。OFF: 各スライスが自分の荷重だけで扁平する。ON では外側の反復が 3〜5 割増える。ロール間の接触（WR–BUR）の扁平はどちらでもスライスごと。').root);
    {
