...
リアルタイムに可視化する。**3D 解析**タブでは 2Hi（2 段ミル）の
ワークロール（上半分）を梁 FEM と Hertz 接触で解き、ロールの撓み・扁平と板の幅方向の板厚・伸び率・張力分布を
リアルタイムに描く（§「3D 解析タブ」。計算エンジンは 4Hi / 6Hi / 12Hi / 20Hi も解けるが、アプリから選べるのは 2Hi だけ）。オープンソースのみで構成（TypeScript + Vite + WebGL2、
外部数値ライブラリなし）。
...
**2026-09-19 から、アプリの 3D タブは 2Hi だけにした。** ミル形式の切り替え（ボタンと `1`〜`5` キー）、プリセット、`?mill=`、
4Hi〜20Hi 専用の入力（ベンダー、IR シフト、IR・BUR・第1/第2中間・バッキング軸の寸法とクラウン、テーパ、AS-U、IR チョックの着座）は
アプリから外した。計算エンジン（`src/sim3d`）は 4Hi / 6Hi / 12Hi / 20Hi をそのまま持っていて、`tools/sim3d` の node の検証と
`tools/frontistr` の照合はそれを使う。この節の 4Hi〜20Hi の記述・測定値・操作の説明は、エンジンの説明か、それより前のアプリの記録として残している。
...
**FrontISTR との照合（アプリからは 2Hi）。** 「解析・表示 ▸ ロールの照合」を FrontISTR にすると、収束のたびに同じ板の荷重を
上半分の WR（ソリッド要素）に載せて FrontISTR（オープンソースの汎用 FEM、手元でビルドした
`fistr1`）で解き、撓み・扁平のグラフに白の破線で重ね、右パネル「FrontISTR 照合」に
中央値と最大差を並べる。`npm run dev` の開発サーバーが橋渡しをする（ブラウザからは `fistr1` を起動できない）。
1 回 約 10 秒。4Hi・6Hi のケースは `tools/frontistr` から node で組める（12Hi・20Hi のクラスタは未対応）。ビルドと結果は `tools/frontistr/README.md`。
...
残っていても効く（Space でそのボタンがもう一度押されたり、トグルが反転したりはしない。2D タブと同じ扱い）。エンジンの既定の 4Hi（BUR クラウン 0、
...
**ロール寸法の上下限。** WR のネック径は WR 直径まで、支持スパンは胴長から下には動かせない。
...
`?tab=3d` で 3D 解析タブを開く（2Hi。2026-09-19 まであった `&mill=` は外した）。`window.__v3` に
