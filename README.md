...
- **側面図**（端面図の下）: 同じ上半分のロールを横（オペレータ側）から見た図。胴・ネックを x・y 同じ縮尺で、ロールのシフト、支持（□ チョック、赤 ▼ 圧下、黄 ● サドル、緑の矢印 ベンダー）、パスラインの板幅とともに描く。端面図で同じ高さのロールは重なり、ミル中心線に近いロールが手前。撓みは描かない（正面図）。
