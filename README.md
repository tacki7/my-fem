...
4Hi 1.0 秒・20Hi 2.2 秒。
一様パスで平面 FEM と一致（中央 17.0 vs 17.2 kN/mm、前進すべり 4.4%、圧力分布同形）、層数 1〜3 で不変。
...
（2D タブと同じ Markov の汎関数、Picard 反復）で解く。板厚方向は一様速度の薄板近似で、板厚は未知量で
...
出側と板端は自由で前後方張力を境界荷重に置く。ロール周速を速度の尺度とし（速度非依存材料）、
...
伸びを FEM との差で補正する。補正は Newton が静定したときだけ更新し、補正が動かなくなった状態が
...
一様パスでの検算（`tools/sim3d/femtest.mjs`）: 出側の圧力 1.155 k̄f、境界で σ_z ≈ 0、荷重は
スラブ法比 +22%（μ 0.06）〜 +31%（μ 0.15）、質量収支 0.99〜1.00、暖機始動で 1〜3 反復 3 ms。
...
0.08 秒、平面 FEM は 25 反復 0.11 秒、node で計測）。
...
扁平に返る。OFF にすると張力なしの圧延として計算するので効果が比較できる: 既定 4Hi で荷重 1729 → 1905 tonf、
C25 114 → 163 µm。統計に「張力による降伏緩和 σ̄t/k̄f」を出す。
...
- **左右非対称に撓む。** レベリングがなければ、解いた下半分の撓みは上半分を左右反転したものに一致し
  ...
  高くなる。既定で WR–IR 間は −x 側の胴端の 1.6 kN/mm から IR 胴端の手前（+471 mm）の 17 kN/mm まで上がり、WR は胴長にわたって
  約 1 mm 傾く。BUR の左右の支持反力は等しい（浮いた IR が荷重の合力を中央へ戻す）。
...
- **推定残り時間**（上部の「反復中」チップ）: 収束までの実時間を、ここまでの反復から外挿して「残り 約 N 秒」と出す
  ...
  中央値で残り時間の 25%（描画 16 ms/フレーム）〜33%（60 ms）。収束しない 3 本は全部「不明」になり、
...
ベンダー 0）はわざとクラウン過大で、耳波（顕在形状 6000 I-unit）が出る。WR ベンダー
120 tonf/チョック、または BUR クラウン 400 µm あたりで平坦になる。6Hi の既定（IR 胴端 = 板端）は
軽い耳波（顕在 280 I-unit）で、IR シフト −50 mm（胴端を板端の 50 mm 内側）で平坦になる。プリセット（冷間タンデム 4Hi ／ 薄板 6Hi ／ ステンレス 20Hi ／ 箔 20Hi）は左上。
