...
キーが効くのは 2D タブを表示しているときだけ。入力欄・ボタン・セレクトにフォーカスがあるとき
（Space はボタンの押下やセレクトの展開になる）と、⌘・Ctrl・Alt 付きのとき（ブラウザのショートカット）は
何もしない。以前は 3D タブの表示中にも隠れた 2D が一時停止・リセット・スタンド切替に反応していた。
