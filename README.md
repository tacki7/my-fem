...
| 20Hi | WR, 第1中間×2, 第2中間×3, バッキング軸×4（1-2-3-4） | + 第1中間のテーパ部シフト（板端基準の位置・長さ・深さ）、ダブル AS-U（A–B 軸の組と C–D 軸の組を独立に 7 点ずつ） |
...
動く）。上 WR は +x 側の板端より外で、下 WR は −x 側の板端より外で IR の支持を失う。上下が鏡像では
