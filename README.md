...
（上半分）を梁 FEM と Hertz 接触で解き、ロールの撓み・扁平と板の幅方向の板厚・伸び率・張力分布を
...
対称面なので、板の出側板厚は「上 WR 表面の高さ × 2」。上下で逆向きに動くアクチュエータ
（6Hi の IR シフト）は半モデルでは両端対称に扱う（後述）。
...
| 6Hi | WR, IR, BUR | + IR ベンダー、IR シフト（胴端を板端の内外へ）、IR クラウン |
...
6Hi の IR シフトは、上 IR を +x に、下 IR を −x にずらすものだが、半モデルでは片側しか見えない
ので「両胴端が板端 + シフト量」の対称な胴として扱う。板端外の WR 支持を外す効果（エッジ部の
荷重が落ちてエッジドロップが減る）は出る。片側だけの非対称（ウェッジ）は出ない。
...
最初から平坦。プリセット（冷間タンデム 4Hi ／ 薄板 6Hi ／ ステンレス 20Hi ／ 箔 20Hi）は左上。
...
- 上半分のみ: 上下非対称なもの（片側 IR シフト、上下のロール径差、パスライン高さ）は扱えない。
...
src/
  ...
  sim3d/
    ...
    band.ts     帯 Cholesky（単位ベクトル右辺の短縮版つき）、小さな密 LU
