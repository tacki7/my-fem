...
圧力マップは平面 FEM と同じ形で返し、ロールとの連成（欠陥補正）はそのまま。既定 35 列 × 8 行 × 2 層で
未知数 2,800・半バンド幅 95。Picard 反復ごとの線形解は、以前の Cholesky 分解を前処理にした共役勾配法で解き
（分解済み行列での求解は分解の 1/25、共役勾配法が 12 反復を超えたときだけ分解し直す）、連成の冷間始動は
4Hi 1.2 秒・20Hi 2.0 秒。
...
*平面 FEM（既定）。* 噛み込み域の板を**幅方向 × 圧延方向**に Q4 要素で分割し（幅方向はロールモデルの
板上の分割点、圧延方向は nz 行、既定 8）、定常の速度場 u = (u_x, u_z) を剛塑性の流れ定式化
...
幅あたりの張力と流量は、メッシュ上で各列が受け持つ幅（隣の列までの距離の半分ずつ）で節点に配る。
...
伸びを FEM との差で補正する。荷重の比はスライスの不動点の中に入れる（q = k·P(h₁(q))）ので、WR の扁平と
...
0.04 秒、平面 FEM は 37 反復 0.16 秒、node で計測）。
...
扁平に返る。OFF にすると張力なしの圧延として計算するので効果が比較できる: 既定 4Hi で荷重 1478 → 1652 tonf、
C25 62 → 97 µm。統計に「張力による降伏緩和 σ̄t/k̄f」を出す。
...
- **推定残り時間**（上部の「反復中」チップ）: 収束までの実時間を、ここまでの反復から外挿して「残り 約 N 秒」と出す
  ...
  （補正が周期的に回って収束しない）は「不明」。記録した解 38 本（うち収束 35 本）をフレームごとに再生すると、推定と実際の差は
  中央値で残り時間の 23%（描画 16 ms/フレーム）〜30%（60 ms）。収束しない 3 本は全部「不明」になり、
  収束する解で「不明」が出たことはない（`tools/sim3d/eta.mjs`）。
...
150 tonf/チョック、または BUR クラウン 800 µm あたりで平坦になる。6Hi の既定（IR 胴端 = 板端）は
ほぼ平坦（顕在 20 I-unit）で、IR シフト −25〜−50 mm（胴端を板端の内側へ）で完全に平坦になる。プリセット（冷間タンデム 4Hi ／ 薄板 6Hi ／ ステンレス 20Hi ／ 箔 20Hi）は左上。
