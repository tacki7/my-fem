...
出側板厚が噛み込みの継続限界を下回ると、反力を 0 にできる送り速度そのものが無くなる（入側面が板を押し込み続ける）。
それでも荷重と h₁ は静止するので整定したように見える。送り残差の底が不感帯の 30 倍を超えたまま 300 回解いたら、
ミルライン図の見出しに「⚠ 送り速度の自走が成り立っていない」と出し、「圧延諸元」の送り残差を赤にする（計算は変えない）。
実測で、自走できている条件の残差は不感帯の 2.4 倍以下、中板 8 mm の指令 45% 以上では 280 倍以上
（`docs/validation.md`「締め込みと Stone の比」、検査は `node tools/sim2d/feed.mjs`）。
