...
未知数 2,800・半バンド幅 95、暖機 1 反復 20〜70 ms、連成の冷間始動 10 秒前後（平面 FEM の 10 倍）。
...
連成問題の解（減衰 0.3 — 荷重→扁平→ギャップ→荷重 の利得が 1 を超え、無減衰では 2 周期で振動した）。
...
これらと食い違い、外側 Newton の歩幅を縮めるほうが高くついた）。速い（既定 4Hi で 36 反復
0.2 秒、平面 FEM は 303 反復 1.2 秒）。
...
**圧下位置。** 出側板厚（圧下率）一定／荷重一定モードでは、内側の Newton が静定するたびに
スクリュー S を割線法で 1 歩動かす。数フレームで目標に乗る。負の S（開き）も許す — AS-U や
クラウンでスタックが予圧されていると軽圧下では必要になる。
...
src/
  ...
  sim3d/
    ...
    solver.ts   梁 + Hertz 接触 + 板スライスの Newton、張力の能動集合解法、スクリューの割線法
