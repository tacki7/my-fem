...
20Hi 上半分（10 本、81 点）で 3240 自由度・半バンド幅 43、帯 Cholesky は 1 回 10 ms 弱。
...
これらと食い違い、外側 Newton の歩幅を縮めるほうが高くついた）。速い（既定 4Hi でスラブ法 12 反復
0.04 秒、平面 FEM は 31 反復 0.12 秒、node で計測）。
...
扁平に返る。OFF にすると張力なしの圧延として計算するので効果が比較できる: 既定 4Hi で荷重 1480 → 1655 tonf、
C25 51 → 85 µm。統計に「張力による降伏緩和 σ̄t/k̄f」を出す。
...
落ち込みが出る）。正面図・3D とも隙間で軸が見える。20Hi 既定でクラウン 45 → 47 µm。
...
1227 → 978 µm、IR 753 → 363 µm（docs/validation.md「ハウジング変形考慮モード」）。結果の `housing` に左右の枠の
...
- **左右非対称に撓む。** レベリングがなければ、解いた下半分の撓みは上半分を左右反転したものに一致し
  ...
  高くなる。既定で WR–IR 間は −x 側の胴端の 1.3 kN/mm から IR 胴端の手前（+471 mm）の 15 kN/mm まで上がり、WR は胴長にわたって
...
- **格子。** 節点は IR のネックまで張る。既定（板幅 1000 mm、IR 胴長 1700 mm）では IR が 350 mm ずれて
  格子の半幅が 1175 → 1450 mm に伸び、81 点のままだと分割間隔は 29 → 36 mm になる。
...
- **数値**: 圧延荷重、圧下位置、平均・中央板厚、クラウン C25（中央 − 板端 25 mm 内の平均）、
  ...
  （サドルは 7 点それぞれ）、反復回数・残差・解法時間・自由度。ハウジング変形考慮モードで結果があるときだけ「ハウジング」の節:
...
ベンダー 0）はわざとクラウン過大で、耳波（顕在形状 1100 I-unit）が出る。WR ベンダー
120〜150 tonf/チョック、または BUR クラウン 800 µm あたりで平坦になる。6Hi の既定（IR 胴端 = 板端）は
ほぼ平坦（顕在 4 I-unit）で、IR シフト −25 mm（胴端を板端の内側へ）から完全に平坦になる。プリセット（冷間タンデム 4Hi ／ 薄板 6Hi ／ ステンレス 20Hi ／ 箔 20Hi）は左上。
...
- ロール扁平は各節点で局所（Winkler 型）。周方向には Hertz 圧力分布（断面 FEM ではその畳み込み）を
  ...
  板端のエッジドロップは分割点 1 個分の解像度でしか出ない。材料 FEM の板端の効果（板端で荷重が抜ける、
  横流れ）も 1 セルより細かくは解けず、板端がセルのどこにあるかで結果が連続的に動く: 板幅 990〜1040 mm で
  4Hi の C25 が平面 FEM 48.7〜66.9 µm、3D FEM 57.6〜63.2 µm（`tools/sim3d/widthsweep.mjs`）。
