...
20Hi 上半分（10 本）は 81 点で 3240 自由度・半バンド幅 43、帯 Cholesky は 1 回 10 ms 弱（既定の 301 点では 12040 自由度）。
...
**幅方向の節点。** 節点は、いちばん長いロールの支持スパン（シフトで外へ出た分を含む）にわたって等間隔に
「解析・表示 ▸ 幅方向 分割数」（既定 301）点置き、セルが板に掛かる節点が板のスライスになる。既定の板幅 1000 mm で
板上は 2Hi 143・4Hi 129・6Hi 105・12Hi と 20Hi 195 点（間隔 5.2〜9.7 mm）。以前の既定 81 点では 39・35・29・53 点で、
セル 1 つが 19〜36 mm あった。等間隔の格子では板端のセルは板幅しだいで一部しか板に掛からず、板端がセルのどこにあるかで
板端の結果が動く（「既知の制限」）。「板・圧延条件 ▸ 材料 幅方向 分割数」を N にすると、板を幅 w/N の N 個のセルで
ちょうど敷き詰め（奇数に丸めて中央に節点、板端はセルの境界）、板の外は等間隔の格子と同じくらいの間隔で支持点まで伸ばす
（`src/sim3d/grid.ts`）。全ロールと板が同じ節点を使うことは変わらないので、帯行列もスライスの計算もそのままで、
梁要素の長さ・セル幅・位置から節点を探す処理だけが節点ごとになる。格子は右半分を作って鏡映するので左右ビット単位で対称。
0 では以前の等間隔格子そのもので、結果は 1 ビットも変わらない（既定は下の「分割の既定」）。計算時間は点数より速く伸びる（以下は板上が等間隔の格子のとき）: 81 → 301 点で
1 反復あたり 7〜9 倍（node で 4Hi 2.3 → 19 ms、6Hi 2.9 → 21 ms、20Hi 16 → 136 ms。ブラウザでダイヤルを動かしたあとの再収束は
4Hi 0.06 → 0.65 秒、20Hi 0.44 → 2.9 秒、20Hi の解いている間の 1 フレームは 200〜350 ms）。張力の幅方向の連成
（Woodbury 更新が「スライス数 × 全自由度」、能動集合法の密行列が「スライス数の 3 乗」）が支配するので、
板上の点数を同じにしてロール側だけ粗くしてもほとんど速くならない（81 点 + 板上 129 点で 4Hi 0.63 秒、301 点で 0.65 秒）。
...
今の荷重から更新する（反復 3〜5 割増）。板の外の胴は荷重を受けないので、一様荷重でも板端の扁平は内部の約 53 % になる。
OFF → ON は、4Hi（301 点、板上は等間隔の格子、張力は前後の平均、座屈限界で頭打ち）でエッジドロップ 39.8 → 94.2 µm、C25 53.7 → 86.2 µm、潜在形状 1481 → 2847 I-unit（板端の手前の落ち込みがほぼ消え、
板端約 100 mm にわたって高い）。6Hi（同じ条件）: 潜在形状 111 → 751 I-unit（板端がほぼ平坦 → 耳波）。板の中央部（x ≤ 300 mm）と、
座屈上限の境目の位置は変わらない。ロール間の接触（WR–BUR など）の扁平はスライスごとのまま。docs/validation.md「WR と板の扁平の幅方向の広がり」。
...
（スライス 1 本に要素 1 列、節点列はスライスの境界と板端）。4Hi・幅方向 81 点（板上 35 列）× 8 行 × 2 層で
未知数 2,900・半バンド幅 95。Picard 反復ごとの線形解は、以前の Cholesky 分解を前処理にした共役勾配法で解き
（分解済み行列での求解は分解の 1/25、共役勾配法が 12 反復を超えたときだけ分解し直す）、連成の冷間始動は
81 点で 4Hi 1.0 秒・20Hi 1.7 秒、301 点（板上は等間隔の 129 列・195 列、圧延方向 8）で 3.6 秒・7.1 秒（node）。
...
スライス 1 本に要素 1 列で、節点列はスライスの境界、両端は板端に置く。圧延方向は nz 行、既定 8）、
...
これらと食い違い、外側 Newton の歩幅を縮めるほうが高くついた）。速い（4Hi でスラブ法 12 反復、
平面 FEM は 31〜34 反復。node で 81 点ならスラブ法 0.04 秒・平面 FEM 0.12 秒、301 点で 0.34 秒・0.79 秒。いずれも板上は等間隔の格子・扁平は局所・張力は前後の平均・座屈限界で頭打ちのとき。今の既定の時間は「分割の既定」）。
...
扁平に返る。OFF にすると張力なしの圧延として計算するので効果が比較できる: 既定 4Hi で荷重 1476 → 1650 tonf、
C25 54 → 88 µm（81 点では 1480 → 1655 tonf、51 → 85 µm）。統計に「張力による降伏緩和 σ̄t/k̄f」を出す。
...
スラブ法では潜在形状が増え（301 点・板上は等間隔の格子・扁平は局所・座屈限界で頭打ちで、4Hi 5717 → 6347 I-unit、6Hi 1164 → 1707）、材料 FEM では荷重を FEM が決めるので形状が数 % 動くだけ
（4Hi 1481 → 1529、荷重 1475.9 → 1473.3 tonf）。1 回の収束は平面 FEM で 1.3〜1.9 倍の時間。
...
コイラー設定に保つオフセット）。座屈限界 −σcr 以下の圧縮は支えられず**波**（顕在形状）に、
降伏近く（0.7 kf）以上は伸びて頭打ち。張力は局所荷重を下げ、荷重は扁平を通じて板厚を変え、
板厚は伸びを変える — この**板内ループの利得は −20 前後**で、素朴に反復すると発散する。
...
座屈域の張力も能動集合法のまま Newton で解く。4Hi（301 点、板上は等間隔の格子、扁平は局所、張力は前後の平均）では、中央部の張力分布（帰還の生きた域）は変わらず、変わるのは座屈域の高さ:
- 潜在形状 1481 → β 0.02: 972、β 0.05: 681、有効幅 k 1: 774 I-unit
- C25 53.7 → 47.1 / 43.4 / 43.9 µm
...
落ち込みが出る）。正面図・3D とも隙間で軸が見える。20Hi 既定でクラウン 30.6 → 31.6 µm（分割 OFF → ON）。
...
1228 → 979 µm、IR 750 → 370 µm（81 点では 1227 → 978、753 → 363。docs/validation.md「ハウジング変形考慮モード」）。結果の `housing` に左右の枠の
...
- **格子。** 節点は IR のネックまで張る。既定（板幅 1000 mm、IR 胴長 1700 mm）では IR が 350 mm ずれて
  格子の半幅が 1175 → 1450 mm に伸び、同じ点数のままだと分割間隔は 7.8 → 9.7 mm（既定の 301 点。81 点では 29 → 36 mm）になる。
  「材料 幅方向 分割数」を指定すれば、板上の間隔はシフトに依らない。
...
- **推定残り時間**（上部の「反復中」チップ）: 収束までの実時間を、ここまでの反復から外挿して「残り 約 N 秒」と出す
  ...
  （補正が周期的に回って収束しない）は「不明」。記録した解 38 本（全部収束）をフレームごとに再生すると、推定と実際の差は
  中央値で残り時間の 30%（描画 16 ms/フレームでも 60 ms でも）、90% 点で 92〜96%。30% 以上外れたフレームの
  7 割は実際より長めの推定（補正の変化量が見込みより速く減る）。収束する解で「不明」が出たことはない。
...
ベンダー 0）はわざとクラウン過大で、耳波（顕在形状 1400 I-unit）が出る。WR ベンダー
150 tonf/チョック（120 では顕在 61 I-unit）、または BUR クラウン 800 µm で平坦になる。6Hi の既定（IR 胴端 = 板端）は
ほぼ平坦（顕在 35 I-unit）で、IR シフト −25 mm（胴端を板端の内側へ）から完全に平坦になる（いずれも既定の 301 点）。プリセット（冷間タンデム 4Hi ／ 薄板 6Hi ／ ステンレス 20Hi ／ 箔 20Hi）は左上。
...
- ロール扁平は既定では各節点で局所（Winkler 型）。周方向には Hertz 圧力分布（断面 FEM ではその畳み込み）を
  見ているが、**幅方向**の非局所な弾性沈み込み（接触幅の数倍に広がる）は、WR と板の間だけ「扁平の幅方向の広がり」を ON にしたときに入る
  （ロール間の接触は常に局所）。OFF では板端のエッジドロップは板上の節点 1 個分の解像度でしか出ない。材料 FEM の板端の効果（板端で荷重が抜ける、
  横流れ）も 1 セルより細かくは解けない。等間隔の格子（「材料 幅方向 分割数」0）では板端がセルのどこにあるかで結果が
  連続的に動く: 板幅 990〜1040 mm で 4Hi 平面 FEM のエッジドロップが既定の 301 点で 38.5〜43.8 µm（C25 53.2〜55.6 µm。
  81 点では C25 48.7〜66.9 µm）、3D FEM で 46.1〜49.9 µm。「材料 幅方向 分割数」を 129 にすると板端は常にセルの境界に
  乗り、エッジドロップは平面 FEM 42.4〜42.8 µm・3D FEM 49.6〜50.2 µm に収まる（`tools/sim3d/widthsweep.mjs`）。
...
- 断面 FEM の扁平は、接触半幅が胴表面の節点間隔と同じ程度だと分割数に敏感（既定の周方向 400 分割、R 250 mm で
  節点間隔 3.9 mm。半幅 0.5〜2 mm では Johnson 式との比が 0.82〜1.02 と動き、1600 分割で 0.92 付近に収まる）。
  小径ロールや軽荷重の接触では周方向分割を増やす。
