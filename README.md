...
右パネル「解析 ▸ 幅方向 節点」に板上の点数と間隔 / 全節点数を出す（3D 図の左上の「分割数」にも同じ点数）。docs/validation.md「幅方向の節点」。
...
- **3D 表示**（既定）: 陰影付きの円筒で上半分のスタック、撓みは曲がりだけを倍率表示（平行移動分は除く）、
  ...
  左上の「分割数」は今の条件の分割（正面図でも出す。計算前から条件どおり）: ロールは幅方向の全節点数（板の上を
...
- **チャート**（横軸は幅方向 mm）: ロール撓み v(x) ／ 接触ごとの扁平量 ／ 接触線荷重 q(x) ／
...
- **推定残り時間**（上部の「反復中」チップ）: 収束までの実時間を、ここまでの反復から外挿して「残り 約 N 秒」と出す
...
- **警告**（ステータス行の琥珀色チップ）は 2 種類。**設定だけで決まる警告**（`張力が降伏に近い`・`ロール配置が成立していない`・
