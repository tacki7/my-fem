...
荷重一定で同じステップが 21 s で整定したり 90 s かけても整定しなかったりする原因。
噛み込み入口が板メッシュの列境界に乗ると、その列が接触に入る → 荷重が上がる → 扁平でロールが
h₀/2 より浮いて列が抜ける → 荷重が下がる → また入る、を制御ループの外で繰り返す
（制御 off・剛体ロールで止まり、連成 ON で鳴る）。既定 100×8 で荷重 ±5%、接触 19↔20 列。

ゲイン・不感帯では止まらない。**目標をわずかにずらして入口を列境界から外す**のが確実で、
必要な量（1 列ぶん ≈ Δh₁ 35 µm、半分で外れる）は AGC パネルの説明に実測値から出る。
メッシュを上げると段は低く・出にくくなるが、列境界はどのメッシュにもあるので消えはしない
...
- **スラブ法**: μ逆算が逆に解いているのと同じ閉じた式
  `p̄ = kf*·(e^a−1)/a, a = μL/h̄, kf* = kf − (σb+σf)/2, L = √(R′Δh)`（Hitchcock 扁平込み）を、
  **FEM が実際に作っている出側板厚**に対して評価した値
...
既定条件 3 スタンドでの比（FEM / スラブ法）は 0.81 / 0.64 / 0.55 — スラブ法は均一圧縮を仮定するので
圧下率と摩擦丘が立つほど過大評価する（`docs/validation.md` の圧下率スイープと同じ性質）。
スラブ法モードで荷重一定 1300 tonf を指令すると 11 s で整定する。
...
合わせると細い青線が出る。ダブルクリックでその境界が既定に戻る。サイズはブラウザに保存されるので
次回も維持される。
