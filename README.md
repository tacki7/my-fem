...
リアルタイムに可視化する。**3D 解析**タブでは 2Hi / 4Hi / 6Hi / 12Hi / 20Hi のロールスタック
（上半分。IR をシフトした 6Hi は下半分も）を梁 FEM と Hertz 接触で解き、ロールの撓み・扁平と板の幅方向の板厚・伸び率・張力分布を
リアルタイムに描く（§「3D 解析タブ」）。オープンソースのみで構成（TypeScript + Vite + WebGL2、
外部数値ライブラリなし）。
...
**FrontISTR との照合（2Hi・4Hi・6Hi）。** 「解析・表示 ▸ ロールの照合」を FrontISTR にすると、収束のたびに同じ板の荷重を
上半分のロール（ソリッド要素、ロール同士は接触解析）に載せて FrontISTR（オープンソースの汎用 FEM、手元でビルドした
`fistr1`）で解き、撓み・線荷重・扁平（2Hi）・出側プロファイルのグラフに白の破線で重ね、右パネル「FrontISTR 照合」に
中央値と最大差を並べる。`npm run dev` の開発サーバーが橋渡しをする（ブラウザからは `fistr1` を起動できない）。
2Hi は約 10 秒、4Hi・6Hi は粗い網で数分（12Hi・20Hi のクラスタは未対応）。ビルドと結果は `tools/frontistr/README.md`。
...
**ハウジング変形考慮モードの操作**（左パネル「ハウジング」）。モードの切り替え、6Hi では IR チョックの着座の切り替えと
座の剛性、枠の寸法（ポストの断面積・本数・長さ・幅、クロスヘッドのスパン・断面二次モーメント・せん断断面積、ヤング率）。
...
12Hi・20Hi ではモードの切り替えが無効表示になり、理由を出す（ON のまま形式を変えると、警告を出して今の支持で解く）。
寸法と座の剛性の既定は仮定。プリセットは形式の既定から始めるのでモードは OFF に戻り、形式の切り替えでは設定を引き継ぐ。
...
- **正面図**（切替）: ロールを積み順に並べ、撓みを倍率表示、胴の色はその位置の接触線荷重、▲ は支持点。
  下に板の出側プロファイル（自動スケール）。
- **端面図**: クラスタ配置の円、接触線の太さと色が接触力、数字は tonf。右パネルの図（端面図・側面図・ワークロール断面）は、
...
- **側面図**（端面図の下）: 同じ上半分のロールを入側から（圧延方向に）見た図、左が操作側（−x）。胴・ネックを x・y 同じ縮尺で、ロールのシフト、支持（□ チョック、赤 ▼ 圧下、黄 ● サドル、緑の矢印 ベンダー）、パスラインの板幅とともに描く。端面図で同じ高さのロールは重なり、ミル中心線に近いロールが手前。撓みは描かない（正面図）。
  ...
  上クロスヘッド、クロスヘッドからチョックへの圧下（赤）、6Hi の着座 ON では IR チョックと BUR チョックの間の座（橙 = 荷重を受けている、
  破線 = 浮いている）。パスラインの下にポスト内面の間隔の寸法線。板幅がそれより広いと、ポストと寸法線が赤になり
  `⚠ 板幅 … > ポスト内面 …` と出る（警告チップも出る）。上に左右それぞれの窓の開きと上下クロスヘッドのたわみ（µm）、
  圧下ロールの支持の傾き（駆動側 − 操作側）を数値で出す。開きは数百 µm で枠は数 m あるので、変形の形は描かない。
  IR シフトで開きが左右で等しいまま上下のたわみが入れ替わる様子と、着座 ON で傾きが出る様子が数字で読める。
  モデルはロールをチョックごとシフトするので、IR シフトが大きいと IR の胴がポストに重なって描かれる（描画だけの話で、計算には入らない）。
...
- **数値**: 圧延荷重、圧下位置、平均・中央板厚、圧下率 実績（中央: 1 − h₁中央/h₀、平均: 1 − h₁平均/h₀平均）、クラウン C25（中央 − 板端 25 mm 内の平均）、
  ...
  ハウジング荷重・窓の開き（ポスト・上下クロスヘッドの内訳）の操作側 / 駆動側、圧下ロールの傾き、IR チョック座の力（上・下）、
  ミル剛性（荷重 ÷ 窓の開きの平均）。モード OFF と範囲外の形式では節ごと出さない（右パネルは従来と同じ）。
...
`1`〜`5` でミル形式。キーは、文字を打つ欄（数値ボックス・セレクト）にフォーカスがあるときと、Tab でたどり着いたボタン・トグルにフォーカスが
...
6Hi の既定（IR 胴端 = 板端）も耳波（顕在 436 I-unit）で、IR シフト −70 mm（胴端を板端の内側へ、5 mm 刻みで −65 mm では顕在 3 I-unit）から完全に平坦になる。プリセット（冷間タンデム 4Hi ／ 薄板 6Hi ／ ステンレス 20Hi ／ 箔 20Hi）は左上。
ミル形式のボタン（と 1〜5 キー）は他の設定を変えない: 板・圧延条件、制御、アクチュエータ、プロファイル、
解析設定はそのままで、ロール寸法だけがその形式のものに替わる（形式ごとに記憶、初回は既定寸法）。同じ条件で
形式を比べるための動作。形式に合った典型条件ごと切り替えたいときはプリセットを使う（こちらは全部を設定する）。

**ロール寸法の上下限。** ネック径（バッキング軸径）はロール直径（バッキング外径）まで、支持スパンは胴長から下には動かせない。
スライダーの端がそこになり、それを超える数値を打ち込んでも ▴▾ で送っても端で止まる。
ロール直径を細くするとネック径が、胴長を支持スパンより長くすると支持スパンが追従する。プリセットと形式ごとに記憶した寸法にも同じ上下限を掛ける
（プリセット「箔 20Hi」は WR 直径 40 mm に既定の WR ネック径 55 mm が残ってロールより太かったので、20Hi 既定の比率の 34 mm にした）。

**圧下率の定義とプリセットの入側クラウン。** 出側板厚一定の目標は「出側の幅平均の板厚 = (1 − 圧下率) × h₀」で、h₀ は板幅中央の入側板厚。
入側クラウンがあると入側の幅平均は h₀ − C/3 なので、幅平均で見た実際の圧下率はダイヤルより少し小さい（右パネル「荷重・圧下 ▸ 圧下率 実績 中央 / 平均」に両方を出す）。
既定の入側クラウン 30 µm はどの板厚でも同じなので、薄板のプリセットではこのずれが大きかった（箔 20Hi: 0.1 mm に 30 µm のクラウン = 30 %、ダイヤル 20 % に対して平均 11 %）。
プリセットは入側クラウンを 4Hi 既定と同じ比率（h₀ の 1.5 %、2 µm 刻み）にする: 薄板 6Hi 16 µm、ステンレス 20Hi 8 µm、箔 20Hi 2 µm（冷間タンデム 4Hi は 30 µm のまま）。
形式の既定値（`defaultParams` の 30 µm）と目標の式は変えていない。
IR（20Hi は第1中間）のネック径にもダイヤルを置いた。以前はダイヤルが無く、IR 直径を一度ネック径より細くするとネック径が引き下げられ、直径を戻しても細いまま戻せなかった。
12Hi・20Hi のサドル幅は、サドル間隔（支持長 × 0.92 ÷ 6）の 9 割までに止める（支持長 700 mm ならサドル間隔 107 mm の 9 割、約 97 mm まで。
サドル幅がサドル間隔以上になるとサドルの間に軸受リングが残らない）。
