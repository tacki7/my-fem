...
`1`〜`5` でミル形式。キーは、文字を打つ欄（数値ボックス・セレクト）にフォーカスがあるときと、ボタンにフォーカスがあるとき（Space はボタンの押下）と、
⌘・Ctrl・Alt 付きのときは効かない。トグルやスライダーをクリックした直後は効く（Space でトグルが反転したりはしない）。既定の 4Hi（BUR クラウン 0、
