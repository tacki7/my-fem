...
- **警告**（ステータス行の琥珀色チップ）: `Stone 限界`（目標板厚が Stone の最小板厚
  ...
  （胴からはみ出した板は圧延されず計算にも入らない、はみ出し量を併記）／ `板幅がハウジングのポスト内面の間隔より広い`
...
端面図・側面図がすぐその条件で描き変わる。前回の計算の変形・荷重・チャート・数値は薄く表示し、3D 図の右上に「未計算」と出す。
