...
**板 = 幅方向スライスのスラブ圧延。** 板を分割点ごとのスライスに切り、それぞれを
...
これらと食い違い、外側 Newton の歩幅を縮めるほうが高くついた）。
...
| 20Hi | WR, 第1中間×2, 第2中間×3, バッキング軸×4（1-2-3-4） | + 第1中間のテーパ部シフト（板端基準の位置・長さ・深さ）、AS-U（B・C 軸） |
...
- 板は平面ひずみのスライスの集まり。横流れは「伸び差の平滑化」で入れており、幅方向の
  塑性流れそのものは解いていない。
