...
平面ひずみの 1 パスとして Bland & Ford（Hill の閉形式）+ Hitchcock 扁平で解く:

    q = k̄f (1 − σ̄t/k̄f) √(R'Δh) · Qp,   Qp = 1.08 + 1.79 r μ √(1−r) √(R'/h₁) − 1.02 r

変形抵抗は 2D タブと同じ L(ε+M)^N の平均値。出側板厚は「WR 軸位置から決まるギャップ + WR の
扁平 + 出口での弾性回復（スプリングバック）」。スライスの荷重が WR を押し上げ、ギャップに
対する接線 dq/dg（扁平を通した陰関数微分）が WR の節点に「基礎ばね」として入る。
...
`Space` で一時停止、`R` で再初期化。既定の 4Hi（BUR クラウン 0、ベンダー 0）はわざと
クラウン過大で、耳波（顕在形状 4700 I-unit）が出る。WR ベンダー 120 tonf/チョック、
または BUR クラウン 400 µm あたりで平坦になる。6Hi の既定（IR 胴端 = 板端）は最初から平坦。

### 既知の制限

- 上半分のみ: 上下非対称なもの（片側 IR シフト、上下のロール径差、パスライン高さ）は扱えない。
- ロール扁平は各節点で局所（Winkler 型）。ロール表面の非局所な弾性沈み込み（接触幅の数倍に
  広がる）は入れていないので、板端のエッジドロップは分割点 1 個分の解像度でしか出ない。
...
src/
  ...
  sim3d/
    ...
    strip.ts    スライスの Bland & Ford–Hill 荷重、Hitchcock（Newton）、スプリングバック
    contact.ts  Hertz 線接触の接近量 δ(q) と逆算 q(δ)
