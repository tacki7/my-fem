...
- ロール扁平は各節点で局所（Winkler 型）。周方向には Hertz 圧力分布（断面 FEM ではその畳み込み）を
  見ているが、**幅方向**の非局所な弾性沈み込み（接触幅の数倍に広がる）は入れていないので、
  板端のエッジドロップは板上の節点 1 個分の解像度でしか出ない。材料 FEM の板端の効果（板端で荷重が抜ける、
