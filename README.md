...
0（既定）では以前の等間隔格子そのもので、結果は 1 ビットも変わらない。計算時間は点数より速く伸びる: 81 → 301 点で
...
**扁平の幅方向の広がり（非局所、既定 OFF）。** 上の扁平はスライスごとに自分の荷重だけで決まる（Winkler 型）。
「解析・表示 ▸ 扁平の幅方向の広がり（非局所）」を ON にすると、WR と板の扁平に、隣のスライスの荷重によるへこみを足す
...
4Hi 既定（301 点）: エッジドロップ 39.8 → 94.2 µm、C25 53.7 → 86.2 µm、潜在形状 1481 → 2847 I-unit（板端の手前の落ち込みがほぼ消え、
板端約 100 mm にわたって高い）。6Hi 既定: 潜在形状 111 → 751 I-unit（板端がほぼ平坦 → 耳波）。板の中央部（x ≤ 300 mm）と、
...
81 点で 4Hi 1.0 秒・20Hi 1.7 秒、既定の 301 点（板上 129 列・195 列）で 3.6 秒・7.1 秒（node）。
...
平面 FEM は 31〜34 反復。node で 81 点ならスラブ法 0.04 秒・平面 FEM 0.12 秒、既定の 301 点で 0.34 秒・0.79 秒）。
...
**荷重式の張力（「板・圧延条件 ▸ 荷重式の張力」、既定 前後の平均）。** 既定は上の式のとおり σ̄t = (σb + σf)/2 を弧全体で
...
（2D タブの Orowan 0.34 L・0.91 L、2D FEM 0.27 L・0.92 L）。幅方向に分布する前方張力の効きが弱くなるので、スラブ法では
潜在形状が増え（4Hi 5717 → 6347 I-unit、6Hi 1164 → 1707）、材料 FEM では荷重を FEM が決めるので形状が数 % 動くだけ
...
**座屈した後の剛性（「板・圧延条件 ▸ 座屈後の構成則」「座屈後の剛性比 β」、既定 0）。** 座屈限界に張り付いたスライスは
既定では −σcr で頭打ち（剛性 0）で、超過した伸びは全部波になる。剛性を入れると、座屈域のスライスはその先の伸び差 ΔD の
一部を圧縮で持つ。構成則は 2 通り:
- 一定の剛性比: σ = −σcr − β·E′·ΔD
- 有効幅: |σ| = √(σcr² + k·σcr·E′·ΔD)（von Kármán の有効幅。座屈直後の傾きは E′ の k/2、波が深いほど寝る。圧縮は −0.7 k̄f で止める。両縁を支えた板の値なので、片側が自由縁の板端では余力を多めに見ている可能性がある）

座屈域の張力も能動集合法のまま Newton で解く。既定 4Hi（301 点）では、中央部の張力分布（帰還の生きた域）は変わらず、変わるのは座屈域の高さ:
