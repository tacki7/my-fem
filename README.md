...
| 12Hi | WR, IR×2, バッキング軸×3（1-2-3） | AS-U（中央 B 軸のサドル 7 点）、配置角 |
| 20Hi | WR, 第1中間×2, 第2中間×3, バッキング軸×4（1-2-3-4） | + 第1中間のテーパ部シフト（板端基準の位置・長さ・深さ）、AS-U（B・C 軸） |

クラスタの配置は WR から外へ接円で組む（第1中間は指定角 ±α₁、第2中間の中央は両第1中間に接し、
外側は第1中間と中央に接して詰める、バッキングは同様）。接触の法線は中心を結ぶ向きで、
鉛直荷重が横方向の力を生むので、クラスタのロールは横にも動く（w）。
