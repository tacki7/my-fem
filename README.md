...
**FrontISTR との照合（2Hi）。** 「解析・表示 ▸ ロールの照合」を FrontISTR にすると、収束のたびに同じ板の荷重をソリッド要素の
WR に載せて FrontISTR（オープンソースの汎用 FEM、手元でビルドした `fistr1`）で解き、撓みと扁平のグラフに白の破線で重ね、
右パネル「FrontISTR 照合（2Hi）」に中央値と最大差を並べる。`npm run dev` の開発サーバーが橋渡しをする（ブラウザからは
`fistr1` を起動できない）。2Hi のみ、1 回 10〜20 秒。ビルドと結果は `tools/frontistr/README.md`。
