...
座の剛性、枠の寸法（ポストの断面積・本数・長さ、クロスヘッドのスパン・断面二次モーメント・せん断断面積、ヤング率）。
節の下に、寸法から出した片側の鉛直剛性（上下対称なときの 1 チョック）と、ポストとクロスヘッドの荷重あたりの伸び・たわみを出す。
...
- **側面図**（端面図の下）: 同じ上半分のロールを横（オペレータ側）から見た図。胴・ネックを x・y 同じ縮尺で、ロールのシフト、支持（□ チョック、赤 ▼ 圧下、黄 ● サドル、緑の矢印 ベンダー）、パスラインの板幅とともに描く。端面図で同じ高さのロールは重なり、ミル中心線に近いロールが手前。撓みは描かない（正面図）。ハウジング変形考慮モードでは、圧下ロールの両端の支持位置にハウジング枠の略図（ポストと上クロスヘッド、パスラインの下は破線）を重ね、
  上に左右それぞれの窓の開きと上下クロスヘッドのたわみ、圧下ロールの支持の傾き（駆動側 − 操作側）を数値で出す。
  開きは数百 µm で枠は数 m あるので、変形の形は描かない。IR シフトで開きが左右で等しいまま上下のたわみが入れ替わる様子と、
  着座 ON で傾きが出る様子が数字で読める。
...
- **警告**（ステータス行の琥珀色チップ）: `Stone 限界`（目標板厚が Stone の最小板厚
  ...
  （胴からはみ出した板は圧延されず計算にも入らない、はみ出し量を併記）。
