...
- **チャート**（横軸は幅方向 mm）: ロール撓み v(x) ／ 接触ごとの扁平量 ／ 接触線荷重 q(x) ／
  ...
  入側 − 出側が中央に対する伸び差で、1 % = 1000 I-unit）／ クラウン比率変化（出側 − 入側のクラウン比率、%。0 なら形状を保ち、
  負は板端が中央に比べて入側より薄く圧延された＝板端側が伸びる（耳波の側）、正は中央側が伸びる）／ 伸び率分布（潜在 Δε と顕在＝波、I-unit = 10⁻⁵、各曲線とも最も伸びの小さい幅位置を 0 とした値。板上の節点と板の両端で評価した平滑後の伸びで、統計の潜在・顕在形状も同じ値から取る）／
  前方張力分布 σf(x)。
