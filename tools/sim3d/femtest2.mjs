...
for (let i = 0; i < nx; i++) { H0[i] = h0; H1[i] = h1; L[i] = Larc; sB[i] = tens; sF[i] = tensF; }
const fem = new StripFem();
const r = fem.solve({ edges, h0: H0, h1: H1, L, kf: (i, e) => (KF ? 740e6 : kfAt(law, e)), mu, sigmaB: sB, sigmaF: sF, nz, vRoll: 1 });
