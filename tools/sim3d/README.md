...
node tools/sim3d/paramspath.mjs      # 入力を変える経路: 既定から setParams で変えた解と、最初からその条件で作った解が一致するか（npm run check に入る。約 55 s）
...
モデルは既定（非局所扁平・前後を分けた張力・座屈後の剛性・リングの分割）のまま、分割だけ軽くする: 幅方向 81 点、
板上は等間隔の格子（「材料 幅方向 分割数」0）、圧延方向 8。既定の分割（板上 281〜561 × 16）では関門が 5.7 秒から 4 分を超える。
...
4Hi・20Hi の既定（stations 81 +「材料 幅方向 分割数」101・圧延方向 8。扁平は局所・張力は前後の平均に固定、`postbuckling.mjs` の `MODEL`）で、`postBucklingStiffness` 0 の明示が頭打ち（剛性 0）とビット一致すること、
