...
- 「ロール撓み」に WR の軸の FEM 曲線、「扁平量」に板の下の扁平を白の破線で重ねる
- 右パネル「FrontISTR 照合」に、板中央の WR の撓みと扁平のモデル／FEM、最大差（括弧はモデルの最大値に
  対する比）、軸受反力（FEM）と板の荷重、節点数と所要時間
- 上のチップ「FrontISTR」に WR 中央の差と最大相対差

条件を変えると結果は消え、次の収束でまた解く。3D タブは 2026-09-19 から 2Hi だけなので、アプリから解くのは 2Hi だけ。
橋渡し（`bridge.mjs`）は 4Hi・6Hi のケースも今までどおり組める（上の `case.mjs` と同じ。12Hi・20Hi のクラスタは未対応: 斜めに
並ぶバッキングベアリングとサドルの接触は別の仕事）。板は関門と同じ「ロールの節点に載せる・弧 8 分割」に固定し、
それ以外の条件（寸法・クラウン・板・荷重・扁平モデルなど）はアプリの設定をそのまま使う。
