...
| 1 | 21.83 µm | | 1477.4 | 64.28 | 105.69 | 2290 | −10.04 µm | 323 s |
| 2 | 0.48 | 1.1e-4 | 1477.4 | 64.41 | 105.70 | 2295 | −9.74 | 430 s |
| 3 | 0.04 | 2.9e-5 | 1477.4 | 64.41 | 105.70 | 2295 | −9.71 | 414 s（定常） |

3 回・1168 秒で定常。荷重は +0.01 %、C25 は 68.10 → 64.41 µm（−3.7 µm）、エッジドロップ 112.31 → 105.70 µm、潜在形状 2375 → 2295 I-unit、
スクリュー 2.1669 → 2.1534 mm。δ は板中央で −9.7 µm（ソリッドの WR 表面の方が板に近い）から板端の手前 380 mm で −22 µm。
...
|---|---|---|---|---|---|---|---|
| 既定（板幅 1000 mm） | 3 | 1168 s | +0.01 % | 68.10 → 64.41 µm | 112.31 → 105.70 µm | 2375 → 2295 I | 22.2 µm |
| 板幅 1400 mm | 3 | 1415 s | −0.01 % | 48.17 → 34.03 | 97.69 → 113.62 | 1706 → 2283 | 67.3 |
| WR ベンダー 60 tonf/チョック | 3 | 1834 s | −0.07 % | 56.53 → 43.12 | 97.95 → 80.14 | 1651 → 945 | 21.2 |

どれも 3 回で定常（δ の動き 20〜64 µm → 0.5〜3 µm → 0.04〜0.24 µm）。板幅 1400 mm では板端が胴端（WR・BUR とも 1600 mm）の 100 mm 内側に来て、
ソリッドの胴端の効き（照合の「結果 2」で見た胴端の線荷重の集中）が板端の表面を持ち上げ、δ が 67 µm になる — 梁のモデルとの差がいちばん大きい条件。
