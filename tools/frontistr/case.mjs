...
//
// Writes roll.msh, roll.cnt, hecmw_ctrl.dat and reference.json (the roll model's answer at the
// same stations) into outdir (default tools/frontistr/run/<mill>). run.sh solves it,
...
const { defaultParams, radiusProfile } = await import(new URL('stack.js', B));
...
console.log(`${mill}: ${it} iterations, F ${(R.force / TONF).toFixed(1)} tonf; ${ref.mesh.map((m) => `${m.name} ${m.nodes} nodes (${m.stations} × ${m.layers} × ${m.angles})`).join(', ')} = ${nn} nodes; strip load on the quarter ${(Fsum / TONF).toFixed(2)} tonf (F/4 = ${(R.force / 4 / TONF).toFixed(2)}); ${loads.length} loaded nodes${mB ? `; contact: ${ng.SLAVE.length} slave nodes, ${sg.MASTER.length} master faces` : ''} → ${out}`);
